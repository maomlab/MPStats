...
data <- expand.grid(
  nuclear = c("nuclear", NA),
  cell = c("cell", NA),
  lipid = c("lipid", NA),
  virus = c("virus", NA),
  plate_id = c("plate1", "plate2", "plate3")) %>%
  tidyr::unite("dye_set", nuclear, cell, lipid, virus, sep = "_", na.rm=TRUE) %>%
  dplyr::mutate(
    Zprime = 1 - rgamma(dplyr::n(), shape = .3)) %>%
  dplyr::filter(dye_set != "")

plot_Zprime_by_dye(Zprime_by_plate = data)
