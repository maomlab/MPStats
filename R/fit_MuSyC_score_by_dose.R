...
fit_MuSyC_score_by_dose <- function(
  ...
  s1_prior = brms::prior(normal(1, 3), nlpar = "s1", ub = -.1),
  s2_prior = brms::prior(normal(11, 3), nlpar = "s2", ub = -.1),
  ...
  s1_init = function() {rnorm(1, .1, 1.5)},
  s2_init = function() {rnorm(1, .1, 1.5)},
  alpha_init = function() {rnorm(0, 2)},
